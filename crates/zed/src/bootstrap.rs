...
use crate::zed::{
    self, OpenListener, OpenRequest, OpenRequestKind, RawOpenRequest, app_menus,
    build_window_options, derive_paths_with_position, eager_load_active_theme_and_icon_theme,
...
fn initialize_stcode_layout(cx: &mut App) {
    cx.observe_new(|multi_workspace: &mut MultiWorkspace, window, cx| {
        ...
        apply_stcode_layout_to_workspace(&workspace, window, cx);
        cx.subscribe_in(
            &workspace,
            window,
            |_, workspace, event: &workspace::Event, window, cx| {
                if matches!(event, workspace::Event::PanelAdded(_)) {
                    apply_stcode_layout_to_workspace(workspace, window, cx);
                }
            },
        )
...
fn apply_stcode_layout_to_workspace(
