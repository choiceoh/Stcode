...
- AI Smart Start: start each session from an isolated worktree when possible, route the default New Thread action to lane creation, auto-open the destination agent draft, and surface leftover changes as a handoff task before the next session starts
