...
- [x] Desktop app parity polish.
  - 기본 창 크기, 상단 title/action bar, 비기술 모델 chip, 선택된 새 작업 row, 하단 작업 트리 affordance를 정리.
  - welcome 화면에서 기술 모델명과 설명체 문구 노출을 줄이고 실제 데스크탑 콘솔처럼 보이게 조정.
- [x] Codex Desktop 복붙식 불필요 nav 제거.
  - 검색/플러그인/자동화 메뉴를 제거하고 프로젝트, 작업 세션, 작업 안전망 중심으로 재정리.
- [x] 최근 프로젝트 목록.
  - 프로젝트를 열면 설정에 최근 경로를 저장하고, 첫 화면/사이드바에서 바로 새 작업으로 재개.
  - 첫 화면도 좌측 내비게이션이 유지되는 데스크탑 앱 shell로 정리.
- [ ] 최근 세션 목록.
- [x] 세션별 요약/작업 트리 패널을 코드 보기 없이 제공.
  - 활성 세션 오른쪽에 작업공간, 에이전트, 변경 저장 흐름을 표시.
  - 요청/응답/도구/실패 수와 원본 보호/작업공간/되돌리기 안전망 상태를 표시.
- [ ] 세션 완료 결과 리포트.
